...
class MenuController extends Controller
{
    ...
    {
        //echo "<pre>";
        $menu_info = DB::connection('mysql4')->table('menu')->groupBy('menu_name')->select(['menu_name'])->orderBy('menu_name')->get()->toArray();
        $info = [];
        foreach($menu_info as $k=>$v){
            $sub_menu = DB::connection('mysql4')->table('menu')->where(['menu_name'=>$v->menu_name])->orderBy('menu_name')->get()->toArray();
        ...
        return view('admin.wechat.menuList',['info'=>$info]);
    }
    /**
     * 添加菜单
     */
    public function do_add_menu(Request $request)
    {
        ...
        $data = [];
        $result = DB::connection('mysql4')->table('menu')->insert([
    ...
    {
        $menu_info = DB::connection('mysql4')->table('menu')->groupBy('menu_name')->select(['menu_name,id'])->orderBy('id')->get()->toArray();
        foreach($menu_info as $v){
            $menu_list = DB::connection('mysql4')->table('menu')->where(['menu_name'=>$v->menu_name])->get()->toArray();
//            dd($menu_list);
            $sub_button = [];
